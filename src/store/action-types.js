// 注册
export const SIGNUP = 'SIGNUP';
// 注册成功
export const SIGNUP_SUCCESS = 'SIGNUP_SUCCESS';
// 注册失败
export const SIGNUP_ERROR = 'SIGNUP_ERROR';

// 登录
export const SIGNIN = 'SIGNIN'; // side function
// 登录成功
export const SIGNIN_SUCCESS = 'SIGNIN_SECCESS'; // plain
// 登录失败
export const SIGNIN_ERROR = 'SIGNIN_ERROR'; // plain
// 解析用户
export const LOAD_USER = 'LOAD_USER'; // plain
// 退出登录
export const LOGOUT = 'LOGOUT'; // plain
// 退出成功
export const LOGOUT_SUCCESS = 'LOGOUT_SUCCESS'; // plain

// 添加文章
export const ADD_ARTICLE = 'ADD_ARTICLE'; // side function
// 添加文章成功
export const ADD_ARTICLE_SUCCESS = 'ADD_ARTICLE_SUCCESS'; // plain
// 添加文章失败
export const ADD_ARTICLE_FAILED = 'ADD_ARTICLE_FAILED'; // plain
// 查询所有文章
export const QUERY_ALL_ARTICLE = 'QUERY_ALL_ARTICLE';
// 查询所有文章成功
export const QUERY_ALL_ARTICLE_SECCESS = 'QUERY_ALL_ARTICLE_SECCESS';
// 获取指定id的文章
export const QUERY_CURRENT_ARTICLE = 'QUERY_CURRENT_ARTICLE';
export const QUERY_CURRENT_ARTICLE_SUCCESS = 'QUERY_CURRENT_ARTICLE_SUCCESS';
export const PLUS_THUMB = 'PLUS_THUMB';
export const PLUS_THUMB_SUCCESS = 'PLUS_THUMB_SUCCESS';

// 添加分类
export const ADD_CATEGORY = 'ADD_CATEGORY';
// 查询分类
export const QUERY_CATEGORY = 'QUERY_CATEGORY';
// 添加分类成功
export const QUERY_CATEGORY_SUCCESS = 'QUERY_CATEGORY_SUCCESS';
// 修改分类
export const EDIT_CATEGORY = 'EDIT_CATEGORY';
// 删除分类
export const DELETE_CATEGORY = 'DELETE_CATEGORY';
